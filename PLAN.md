...
> Last updated: **2026-07-18** · Status: **Character-driven immersive 3D journey (Rounds 1–5 done). Hero reworked into a 3D "dev at his desk" (¾ back view on an office chair + laptop, decorative auto-typing terminal, orbit controls). Committing per round, NOT pushing (Sahil's instruction).**
...
- **2026-07-18** — **Round 5 living character** (committed `8230822`, local only): **age progression** `ageAt` — grows child→man across the scroll (root + head-proportion scale; camera zoom/eyeline adapt); **hair grows out with age** (short kid crop → long adult hair via a scaled `longHair` group); **per-phase outfits** `outfitAt` — school uniform → **goalkeeper kit** (green #1 jersey, white gloves w/ orange cuffs, shorts, socks) → casual → suit; backpack only at school + college; **goalkeeper arm spread fixed** (inverted Z-rotation signs); **seated desk pose** (added chair + desk modesty panel); **navbar auto-hides** during the pinned 3D playground and returns after (`App` `journeyActive` ↔ `Journey onActiveChange` ↔ `Header .is-hidden`); labels lowered (3.1→2.75) so headings never clip. All verified via self-screenshots (`PAGE ERRORS: none`).
- **2026-07-18** — **Hero rework: "dev at his desk"** (this commit, local only): replaced the abstract floating shapes with a low-poly **3D "me" on an office chair at a wooden desk with drawers + laptop**, ¾ **back view**, white shirt + jeans (`hero` outfit added to `BoyCharacter`) — `HeroDeskCanvas.jsx` with **OrbitControls** (drag to change the view) and drei **`ContactShadows`** (no hard floor). The laptop screen hosts a **non-interactive, decorative auto-typing terminal** (`DeskTerminal.jsx` — loops `npm run dev` / git / build lines; per Sahil, the interactive one caused UI chaos so it was retired). Canvas **edges feathered** with a radial CSS mask so the scene melts into the page (no dark square). **Hero margin bug fixed** — `.hero__grid` had `width:100%` overriding the `.container` gutter, so the big `<h1>` ran to the edge; removed it (name now aligns with the header wordmark at every width, measured 48/36/16px). Fluid container gutter widened. Mobile/reduced-motion falls back to the DOM `DeskTerminal`. Removed now-dead `HeroCanvas.jsx` (floating shapes) + `Terminal.jsx` (interactive, retired); `Terminal.css` kept (used by `DeskTerminal`). Verified via self-screenshots (`PAGE ERRORS: none`). **Committed locally, NOT pushed.** puppeteer/`package*.json`/`_*.mjs` deliberately left out.
